...
/newsrpm article --id 12345`,
  ...
  execute: async ({ args, agent }: AgentCommandInputType<typeof inputSchema>): Promise<string> => {
    ...
    if ((slug && id !== undefined) || (!slug && id === undefined)) {
      throw new CommandFailedError(formatAgentCommandUsageError(command, "Provide exactly one of --slug or --id."));
    }
    ...
    if (!id) {
      throw new CommandFailedError(formatAgentCommandUsageError(command, "Argument --id must be a non-zero article ID."));
    }
