...
- The chat command exposed to the user will be called /newsrpm, and should be placed in pkg/newsrpm/chatCommands/newsrpm.ts
- Tools should be placed in pkg/newsrpm/tools/{searchIndexedData, searchArticles, getArticleBySlug, getArticleById, uploadArticle, listProviders, getBody, renderBody}.ts
...
- Error handling:
  ...
  - Note: The API schema uses the property name "visiblity" (typo) in many places; mirror field names exactly when calling the API.

Chat command: /newsrpm
- Sub-commands:
  - /newsrpm index <key> [--value <val|csv>] [--count <n>] [--offset <n>] [--min <iso>] [--max <iso>] [--order date|dateWithQuality]
  - /newsrpm search [--publisher <name|csv>] [--provider <name|csv>] [--type <type|csv>] [--fulltext <q>] [--sponsored true|false] [--count <n>] [--offset <n>] [--min <iso>] [--max <iso>] [--language <code>]
