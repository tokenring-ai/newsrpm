...
export type ArticleBodyResponse = { success: boolean; body: { v: number; chunks: Array<{ name: string; format: string; content: string }> } };
...
export default class NewsRPMService extends Service {
  ...
  private async doFetchWithRetry(input: RequestInfo | URL, init: RequestInit & { timeoutMs?: number } = {}): Promise<Response> {
  ...
  async searchIndexedData(body: any): Promise<MultipleArticleResponse> {
    ...
    const res = await this.doFetchWithRetry(url, { method: 'POST', headers: this.buildHeaders(), body: JSON.stringify(body) });
  ...
  async uploadArticle(article: any): Promise<{ success: boolean; id: number }> {
    ...
    const res = await this.doFetchWithRetry(url, { method: 'POST', headers: this.buildHeaders(), body: JSON.stringify(article) });
